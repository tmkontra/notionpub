...
def uploadBlock(blockDescriptor: dict, blockParent: PageBlock, mdFilePath):
    """
    ...
    """
    blockClass: Type[Block] = blockDescriptor["type"]
    ...
    if issubclass(blockClass, EmbedOrUploadBlock):
        ...
        if re.search(r"(?<!file)://", imgRelSrc, re.I):
            # Don't upload images that are external urls
            return
        ...
        raise ValueError("CollectionViewBlock upload is not supported")
        ...
        newBlock.collection = notionClient.get_collection(
            ...
            notionClient.create_record(
                "collection", parent=newBlock, schema=collectionSchema
            )
        )
        view = newBlock.views.add_new(view_type="table")
        for row in collectionRows:
            newRow = newBlock.collection.add_row()
            for idx, propName in enumerate(
                prop["name"] for prop in collectionSchema.values()
            ):
                # TODO: If rows aren't uploading, check to see if there's special
                # characters that don't map to propName in notion-py
                propName = (
                    propName.lower()
                )  # The actual prop name in notion-py is lowercase
                propVal = row[idx]
                setattr(newRow, propName, propVal)
    for childBlock in blockChildren or []:
        uploadBlock(childBlock, newBlock, mdFilePath)
...
def convert(mdFile, notionPyRendererCls=NotionPyRenderer):
    ...
    @param {NotionPyRenderer} notionPyRendererCls Class inheriting from the renderer
    incase you want to render the Markdown => Notion.so differently
    """
    return mistletoe.markdown(mdFile, notionPyRendererCls)


def upload(mdFile, notionPage, notionPyRendererCls=NotionPyRenderer):
    """
    ...
    for idx, blockDescriptor in enumerate(rendered):
        ...
        print(
            f"\rUploading {blockDescriptor['type'].__name__}, {idx+1}/{len(rendered)} ({pct:.1f}%)",
            end="",
        )
        uploadBlock(blockDescriptor, notionPage, mdFile.name)
...
def filesFromPathsUrls(paths):
    ...
    for path in paths:
        if "://" in path:
            r = requests.get(path)
            if (
                not r.status_code < 300
            ):  # TODO: Make this better..., should only accept success
...
def cli(argv):
    ...
    parser.add_argument(
        "token_v2", type=str, help="the token for your Notion.so session"
    )
    parser.add_argument(
        "page_url",
        type=str,
        ...
        if args.mode == "create" or args.mode == "clear":
            ...
            if args.mode == "clear":
                for child in [
                    ...
                    if isinstance(c, PageBlock) and c.title == mdFileName
                ]:
